...
class _BookFolderPageState extends State<BookFolderPage> {
  ...
  Future<void> _summarizePart(
    ...
    setState(() => _summarizingId = session.id);
    try {
      ...
      await const SectionSummarizer().summarize(
        ...
        customPrompt: settings.customPrompt.isEmpty
            ? null
  ...
  void _openReader(String sessionId, {String? tab}) {
    context.push(
      '/reader/$sessionId',
      extra: tab == null ? null : {'tab': tab},
    );
  }
    ...
    return BlocBuilder<BooksCubit, List<Book>>(
      builder: (context, books) {
        ...
        return BlocBuilder<SessionsCubit, List<ReadingSession>>(
          builder: (context, _) {
            final parts = context.read<BooksCubit>().partsFor(widget.bookId);
            final progress =
                ...
                children: [
                  ...
                  if (parts.isEmpty)
                    ...
                    for (var i = 0; i < parts.length; i++) ...[
                      ...
                      SessionCard(
                        session: parts[i],
                        summarizing: _summarizingId == parts[i].id,
